...
- **FR-012**: Система MUST использовать `langchain_postgres.PGVectorStore` как замену `langchain_chroma.Chroma`
...
- **SC-001**: AI-агент отвечает на все вопросы из `POPULAR_QUESTIONS` с качеством не ниже текущего уровня (критерий: каждый из 10 вопросов получает ответ, содержащий релевантные факты из базы знаний; ни один ответ не содержит "информация не найдена" или пустой контент)
