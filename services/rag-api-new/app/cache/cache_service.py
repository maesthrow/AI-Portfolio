...
class CacheService:
    ...
    def _normalize_question(question: str) -> str:
        ...
        # Обрабатываем полное имя "Дмитрий Каргин" и варианты склонения:
        # - "дмитрий каргин" / "дмитрия каргина" (полное имя)
        # - "дмитрий" / "дмитрия" (только имя)
        # - "каргин" / "каргина" (только фамилия)
        s = re.sub(r"\s+(дмитри[йя]?(\s+каргина?)?|каргина?|dmitriy?(\s+kargin)?)\s*$", "", s, flags=re.IGNORECASE)
