...
def try_plan_fast(question: str) -> tuple[QueryPlanV3 | None, str | None]:
    """
    Быстрая проверка шортката + кэша (<5ms).

    Используется для определения, нужен ли LLM-планировщик,
    чтобы не показывать статус "Составляю план поиска..." при cache hit.

    Returns:
        (plan, source) если найден, иначе (None, None).
        source = "shortcut" | "cache"
    """
...
def get_plan_with_cache(
    ...
    """
    # Fast path: shortcut + cache
    plan, source = try_plan_fast(question)
    if plan:
