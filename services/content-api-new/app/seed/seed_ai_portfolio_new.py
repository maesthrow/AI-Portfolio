...
def upsert_one(session, model, identity: dict, payload: dict):
    ...
    if obj is None:
        ...
        session.add(obj)
        session.flush()  # populate autogenerated PK so downstream FKs (e.g. experience_id) resolve
    else:
        changed = False
...
PROFILE_DATA = {
    ...
    "subtitle": "AI-agents • LLM • RAG • Harness • CV • Backend",
    ...
    "hero_headline": "AI Engineer • Python Developer",
    "hero_description": "Строю надежные AI-системы — от идеи до запуска.",
    "current_position": "разрабатываю AI-агентов в Сбере",
}
...
HERO_TAGS_DATA = [
    ...
    {"name": "Harness", "order_index": 40},
    {"name": "CV", "order_index": 50},
...
FOCUS_AREAS_DATA = [
    {
        ...
        "bullets": [
            ...
            "Агентные сценарии и архитектуры (Harness, ReAct, Plan-and-Execute, LangGraph)",
            "RAG-пайплайны: база знаний, retrieval + rerank, контекстная сборка",
...
def seed_experience(session):
    ...
    for idx, data in enumerate(EXPERIENCE_DATA, start=1):
        ...
        # Identify by the natural unique key (company_slug), not the list position.
        # Reordering EXPERIENCE_DATA otherwise reassigns id->slug and collides with uq_experience_company_slug.
        identity = {"company_slug": slug}
